...
def _assert_greater_equal(self, a, b, msg=None):
    if not a >= b:
        standard_msg = "{} not greater than or equal to {}".format(_safe_repr(a), _safe_repr(b))
        self.fail(_format_message(msg, standard_msg))


def _assert_less(self, a, b, msg=None):
    if not a < b:
        standard_msg = "{} not less than {}".format(_safe_repr(a), _safe_repr(b))
        self.fail(_format_message(msg, standard_msg))


def _assert_less_equal(self, a, b, msg=None):
    if not a <= b:
        standard_msg = "{} not less than or equal to {}".format(_safe_repr(a), _safe_repr(b))
        self.fail(_format_message(msg, standard_msg))
...
def _assert_is_instance(self, obj, cls, msg=None):
    if not isinstance(obj, cls):
        if not msg:
            msg = "{} is not an instance of {!r}".format(obj, cls)
        self.fail(msg)


def _assert_in(self, member, container, msg=None):
    if member not in container:
        standard_msg = "{} not found in {}".format(_safe_repr(member), _safe_repr(container))
        self.fail(_format_message(msg, standard_msg))


def _assert_not_in(self, member, container, msg=None):
    if member in container:
        standard_msg = "{} found in {}".format(_safe_repr(member), _safe_repr(container))
        self.fail(_format_message(msg, standard_msg))
...
def _assert_regex(self, text, expected_regexp, msg=None):
    ...
    if not expected_regexp.search(text):
        ...
        msg = "{}: {!r} not found in {!r}".format(msg, expected_regexp.pattern, text)
        self.fail(msg)
