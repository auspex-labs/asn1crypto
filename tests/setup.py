...
package_data = {TEST_PACKAGE_NAME: data_files}
...
class EggInfoCommand(egg_info):
    def run(self):
        egg_info_path = os.path.join(TESTS_ROOT, "%s.egg-info" % TEST_PACKAGE_NAME)
        if not os.path.exists(egg_info_path):
            os.mkdir(egg_info_path)
        shutil.copy2(os.path.join(TESTS_ROOT, "LICENSE"), os.path.join(egg_info_path, "LICENSE"))
        egg_info.run(self)
...
setup(
    ...
    description=("Test suite for asn1crypto, separated due to file size"),
