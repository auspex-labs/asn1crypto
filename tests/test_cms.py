...
class ClearanceTests(unittest.TestCase):
    def test_clearance_decode_bad_tagging(self):
        ...
        self.assertRaises(ValueError, lambda: cms.Clearance.load(rfc_3281_wrong_tagging).native)
...
class CMSTests(unittest.TestCase):
    def test_create_content_info_data(self):
        ...
        self.assertEqual("data", info["content_type"].native)
        self.assertEqual(b"Hello", info["content"].native)
    ...
    def test_parse_content_info_data(self):
        ...
        self.assertEqual("data", info["content_type"].native)
        self.assertEqual(b"This is the message to encapsulate in PKCS#7/CMS\r\n", info["content"].native)
    ...
    def test_parse_content_info_compressed_data(self):
        ...
        self.assertEqual("compressed_data", info["content_type"].native)
        self.assertEqual("v0", compressed_data["version"].native)
        self.assertEqual("zlib", compressed_data["compression_algorithm"]["algorithm"].native)
        self.assertEqual(None, compressed_data["compression_algorithm"]["parameters"].native)
        self.assertEqual("data", compressed_data["encap_content_info"]["content_type"].native)
        self.assertEqual(
    ...
    def test_parse_content_info_digested_data(self):
        ...
        self.assertEqual("digested_data", info["content_type"].native)
        self.assertEqual("v0", digested_data["version"].native)
        self.assertEqual("sha1", digested_data["digest_algorithm"]["algorithm"].native)
        self.assertEqual(None, digested_data["digest_algorithm"]["parameters"].native)
        self.assertEqual("data", digested_data["encap_content_info"]["content_type"].native)
        self.assertEqual(
            b"This is the message to encapsulate in PKCS#7/CMS\n", digested_data["encap_content_info"]["content"].native
        )
        self.assertEqual(
            b"\x53\xC9\xDB\xC1\x6D\xDB\x34\x3B\x28\x4E\xEF\xA6\x03\x0E\x02\x64\x79\x31\xAF\xFB", digested_data["digest"].native
        )
    ...
    def test_parse_content_info_encrypted_data(self):
        ...
        self.assertEqual("encrypted_data", info["content_type"].native)
        self.assertEqual("v0", encrypted_data["version"].native)
        self.assertEqual("data", encrypted_content_info["content_type"].native)
        self.assertEqual("aes128_cbc", encrypted_content_info["content_encryption_algorithm"]["algorithm"].native)
        self.assertEqual("aes", encrypted_content_info["content_encryption_algorithm"].encryption_cipher)
        self.assertEqual("cbc", encrypted_content_info["content_encryption_algorithm"].encryption_mode)
        self.assertEqual(16, encrypted_content_info["content_encryption_algorithm"].key_length)
        self.assertEqual(16, encrypted_content_info["content_encryption_algorithm"].encryption_block_size)
        self.assertEqual(
    ...
    def test_parse_content_info_enveloped_data(self):
        ...
        self.assertEqual("enveloped_data", info["content_type"].native)
        self.assertEqual("v0", enveloped_data["version"].native)
        self.assertEqual(None, enveloped_data["originator_info"].native)
        self.assertEqual(1, len(enveloped_data["recipient_infos"]))
        self.assertEqual("v0", recipient["version"].native)
        self.assertEqual(
            util.OrderedDict(
                [
                    (
                    ...
                    ("serial_number", 13683582341504654466),
                ]
            ),
            recipient["rid"].native,
        )
        self.assertEqual("rsaes_pkcs1v15", recipient["key_encryption_algorithm"]["algorithm"].native)
        self.assertEqual(None, recipient["key_encryption_algorithm"]["parameters"].native)
        self.assertEqual(
        ...
        self.assertEqual("data", encrypted_content_info["content_type"].native)
        self.assertEqual("tripledes_3key", encrypted_content_info["content_encryption_algorithm"]["algorithm"].native)
        self.assertEqual("tripledes", encrypted_content_info["content_encryption_algorithm"].encryption_cipher)
        self.assertEqual("cbc", encrypted_content_info["content_encryption_algorithm"].encryption_mode)
        self.assertEqual(24, encrypted_content_info["content_encryption_algorithm"].key_length)
        self.assertEqual(8, encrypted_content_info["content_encryption_algorithm"].encryption_block_size)
        self.assertEqual(
            b"\x52\x50\x98\xFA\x33\x88\xC7\x3C", encrypted_content_info["content_encryption_algorithm"]["parameters"].native
        )
        self.assertEqual(
        ...
        self.assertEqual(None, enveloped_data["unprotected_attrs"].native)
    ...
    def test_parse_content_info_cms_signed_data(self):
        ...
        self.assertEqual("signed_data", info["content_type"].native)
        self.assertEqual("v1", signed_data["version"].native)
        self.assertEqual(
            [
                util.OrderedDict(
                    [
        ...
        self.assertEqual("data", encap_content_info["content_type"].native)
        self.assertEqual(b"This is the message to encapsulate in PKCS#7/CMS\r\n", encap_content_info["content"].native)

        self.assertEqual(1, len(signed_data["certificates"]))
        certificate = signed_data["certificates"][0]
        with open(os.path.join(fixtures_dir, "keys/test-der.crt"), "rb") as f:
            self.assertEqual(f.read(), certificate.dump())

        self.assertEqual(1, len(signed_data["signer_infos"]))
        signer = signed_data["signer_infos"][0]

        self.assertEqual("v1", signer["version"].native)
        self.assertEqual(
            util.OrderedDict(
                [
                    (
        ...
        self.assertEqual(3, len(signed_attrs))
        self.assertEqual("content_type", signed_attrs[0]["type"].native)
        self.assertEqual("data", signed_attrs[0]["values"][0].native)
        self.assertEqual("signing_time", signed_attrs[1]["type"].native)
        self.assertEqual(datetime(2015, 5, 30, 13, 12, 38, tzinfo=util.timezone.utc), signed_attrs[1]["values"][0].native)
        self.assertEqual("message_digest", signed_attrs[2]["type"].native)
        self.assertEqual(
    ...
    def test_parse_content_info_pkcs7_signed_data(self):
        ...
        self.assertEqual(4, len(signed_attrs))
        self.assertEqual("content_type", signed_attrs[0]["type"].native)
        self.assertEqual("data", signed_attrs[0]["values"][0].native)
        self.assertEqual("signing_time", signed_attrs[1]["type"].native)
        self.assertEqual(datetime(2015, 6, 3, 5, 55, 12, tzinfo=util.timezone.utc), signed_attrs[1]["values"][0].native)
        self.assertEqual("message_digest", signed_attrs[2]["type"].native)
        self.assertEqual(
    ...
    def test_parse_content_info_cms_signed_digested_data(self):
        ...
        self.assertEqual("signed_data", info["content_type"].native)
        self.assertEqual("v2", signed_data["version"].native)
        self.assertEqual(
            [
                util.OrderedDict(
                    [
        ...
        self.assertEqual("digested_data", encap_content_info["content_type"].native)
        self.assertEqual(
            util.OrderedDict(
                [
                    ("version", "v0"),
                    ...
                    ("digest", b"\x53\xC9\xDB\xC1\x6D\xDB\x34\x3B\x28\x4E\xEF\xA6\x03\x0E\x02\x64\x79\x31\xAF\xFB"),
                ]
            ),
            encap_content_info["content"].native,
        )
        ...
        self.assertEqual(0, len(signed_attrs))
    ...
    def test_parse_content_info_smime_capabilities(self):
        ...
        self.assertEqual("smime_capabilities", signed_attrs[3]["type"].native)
        smime_capabilities = signed_attrs[3]

        self.assertEqual(1, len(smime_capabilities["values"]))
        self.assertEqual(7, len(smime_capabilities["values"][0]))
        self.assertEqual(
    ...
    def test_create_role_syntax(self):
        ...
        self.assertEqual(util.OrderedDict([("role_authority", None), ("role_name", "test@example.com")]), rs.native)
