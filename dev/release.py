...
from . import has_tests_package, package_name, package_root
...
def run():
    ...
    git_wc_proc = subprocess.Popen(
        ["git", "status", "--porcelain", "-uno"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=package_root
    )
    ...
    git_tag_proc = subprocess.Popen(
        ["git", "tag", "-l", "--contains", "HEAD"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=package_root
    )
    ...
    twine.cli.dispatch(["upload", "dist/{}-{}*".format(package_name, tag)])
    if has_tests_package:
        twine.cli.dispatch(["upload", "dist/{}_tests-{}*".format(package_name, tag)])
