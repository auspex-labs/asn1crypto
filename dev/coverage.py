...
from . import other_packages, package_name, package_root
...
def _codecov_submit():
    ...
    if env_name == "ci-travis":
        ...
        build_url = "https://travis-ci.org/{}/jobs/{}".format(os.getenv("TRAVIS_REPO_SLUG"), os.getenv("TRAVIS_JOB_ID"))
        query = {
            "service": "travis",
        ...
        build_url = "https://ci.appveyor.com/project/{}/build/{}".format(
        ...
        query = {
            ...
            "job": "/".join(
                (os.getenv("APPVEYOR_ACCOUNT_NAME"), os.getenv("APPVEYOR_PROJECT_SLUG"), os.getenv("APPVEYOR_BUILD_VERSION"))
            ),
        ...
        build_name = "{} {} {}.{}".format(_platform_name(), impl, major, minor)
        ...
        query = {
            ...
            "name": "GitHub Actions {} on {}".format(build_name, os.getenv("RUNNER_OS")),
        }
    ...
    payload = "PLATFORM=%s\n" % _platform_name()
    payload += "PYTHON_VERSION={} {}\n".format(_plat.python_version(), _plat.python_implementation())
    ...
    with open(os.path.join(root, "coverage.xml"), encoding="utf-8") as f:
        payload += f.read() + "\n"
    payload += "<<<<<< EOF\n"
    ...
    headers = {"Content-Type": "text/plain", "x-amz-acl": "public-read", "x-amz-storage-class": "REDUCED_REDUNDANCY"}
...
def _platform_name():
    ...
    if sys.platform == "darwin":
        ...
        if os.path.exists("/etc/os-release"):
            with open("/etc/os-release", encoding="utf-8") as f:
                ...
                if "NAME" in pairs and "VERSION_ID" in pairs:
                    return "{} {}".format(pairs["NAME"], pairs["VERSION_ID"])
        ...
        return "{} {}".format(_plat.system(), _plat.release())
...
def _do_request(method, url, headers, data=None, query_params=None, timeout=20):
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            for key in headers:
                code += "$wc.Headers.add('{}','{}');".format(key, headers[key])
            code += "$out = $wc.UploadFile('{}', '{}', '{}');".format(url, method, tempf_path)
            ...
            stdout, stderr = _execute(
                [powershell_exe, "-Command", code], os.getcwd(), re.compile(r"Unable to connect to|TLS|Internal Server Error"), 6
            )
            if stdout[-2:] == b"\r\n" and b"\r\n\r\n" in stdout:
            ...
            for key in headers:
                args.append("--header")
                args.append("{}: {}".format(key, headers[key]))
            args.append("--data-binary")
            args.append("@%s" % tempf_path)
            args.append(url)
            stdout, stderr = _execute(
                args, os.getcwd(), re.compile(r"Failed to connect to|TLS|SSLRead|outstanding|cleanly|timed out"), 6
            )
    finally:
    ...
    if len(stderr) > 0:
        raise URLError("Error {}ing {}:\n{}".format(method, url, stderr))
...
def _execute(params, cwd, retry=None, retries=0, backoff=2):
    ...
    proc = subprocess.Popen(params, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=cwd)
    stdout, stderr = proc.communicate()
    code = proc.wait()
    if code != 0:
