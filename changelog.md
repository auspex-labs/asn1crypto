...
- Fix `tsp.TimeStampAndCRL` to be a `core.Sequence` instead of a
  `core.SequenceOf` _via @joernheissler_
...
- Fixed convenience attributes on `algos.EncryptionAlgorithm` when the
  algorithm is RC2 _via @joernheissler_
...
- Added Microsoft OID `microsoft_enroll_certtype` (`1.3.6.1.4.1.311.20.2`)
  to `x509.ExtensionId` _via @qha_
- Fixed a few bugs with parsing indefinite-length encodings _via @davidben_
- Added various bounds checks to parsing engine _via @davidben_
- Fixed a bug with tags not always being minimally encoded _via @davidben_
- Fixed `cms.RoleSyntax`, `cms.SecurityCategory` and `cms.AttCertIssuer` to
  have explicit instead of implicit tagging _via @MatthiasValvekens_
- Fixed tagging of, and default value for fields in `cms.Clearance` _via
  @MatthiasValvekens_
...
- Added sMIME capabilities (`1.2.840.113549.1.9.15`) support from RFC 2633
  to `cms.CMSAttribute` _via Hellzed_
...
- Bug Fixes
  ...
  - When copying or dumping a BER-encoded indefinite-length value,
    automatically force re-encoding to DER. _To ensure all nested values are
    always DER-encoded, `.dump(True)` must be called._
