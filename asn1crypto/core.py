...
from .util import create_timezone, extended_datetime, int_from_bytes, int_to_bytes, timezone, utc_with_dst
...
class Asn1Value:
    ...
    def __init__(
        ...
        try:
            ...
            if method is not None:
                if method not in {"primitive", 0, "constructed", 1}:
                    raise ValueError(
                        unwrap(
                            """
    ...
    def __repr__(self):
        ...
        if _PY2:
            return "<{} {} b{}>".format(type_name(self), id(self), repr(self.dump()))
        else:
            return "<{} {} {}>".format(type_name(self), id(self), repr(self.dump()))
    ...
    def debug(self, nest_level=1):
        ...
        if has_parsed:
            ...
            if _PY2 and isinstance(self.native, byte_cls):
                print("{}    Native: b{}".format(prefix, repr(self.native)))
            else:
                print("{}    Native: {}".format(prefix, self.native))
...
class Choice(Asn1Value):
    ...
    def _format_class_tag(self, class_, tag):
        ...
        return "[{} {}]".format(CLASS_NUM_TO_NAME_MAP[class_].upper(), tag)
...
class Concat:
    ...
    def debug(self, nest_level=1):
        ...
        prefix = "  " * nest_level
        print("{}{} Object #{}".format(prefix, type_name(self), id(self)))
...
class _IntegerBitString:
    ...
    def _as_chunk(self):
        ...
        if len(self.contents) == 1:
            ...
            raise ValueError(f"Empty bit string has {unused_bits_len} unused bits")

        if unused_bits_len > 7:
            # Disallowed by X.690 §8.6.2.2
            raise ValueError(f"Bit string has {unused_bits_len} unused bits")
    ...
    def _parse_children(self, recurse=False):
        ...
        try:
            ...
            while again:
                ...
                if field < field_len:
                    _, field_spec, value_spec, field_params, spec_override = cls._precomputed_specs[field] or self._determine_spec(
                        field
                    )
    ...
    def debug(self, nest_level=1):
        ...
        for field_name in self:
            ...
            if child is not VOID:
                print('{}    Field "{}"'.format(prefix, field_name))
    ...
    def _parse_children(self, recurse=False):
        ...
        try:
            ...
            for index in range(0, total_fields):
                ...
                name, field_spec, value_spec, field_params, spec_override = cls._precomputed_specs[index] or self._determine_spec(
                    index
                )
...
class AbstractTime(AbstractString):
    ...
    def _parsed_time(self):
        ...
        if groups["zulu"]:
            ...
            tz = create_timezone(sign * timedelta(hours=int(groups["dhour"]), minutes=int(groups["dminute"] or 0)))

        if groups["fraction"]:
            # Compute fraction in microseconds
            fract = Fraction(int(groups["fraction"]), 10 ** len(groups["fraction"])) * 1000000
...
def _basic_debug(prefix, self):
    ...
    if self.explicit is not None:
        for class_, tag in self.explicit:
            print("%s    %s tag %s (explicitly tagged)" % (prefix, CLASS_NUM_TO_NAME_MAP.get(class_), tag))
        if has_header:
...
def _int_to_bit_tuple(value, bits):
    ...
    result = tuple(map(int, format(value, f"0{bits}b")))
    if len(result) != bits:
        raise ValueError("Result too large: {} > {}".format(len(result), bits))
