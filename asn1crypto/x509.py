...
class Name(Choice):
    ...
    def build(cls, name_dict, use_printable=False):
        ...
        for attribute_name, attribute_value in name_dict.items():
            ...
            if attribute_name == "email_address":
                ...
                value = DirectoryString(name="printable_string", value=PrintableString(attribute_value))
            else:
                value = DirectoryString(name=encoding_name, value=encoding_class(attribute_value))

            rdns.append(RelativeDistinguishedName([NameTypeAndValue({"type": attribute_name, "value": value})]))
    ...
    def human_friendly(self):
        ...
        if self._human_friendly is None:
            ...
            for key in keys:
                ...
                to_join.append("{}: {}".format(key, native_value))
    ...
    def _recursive_humanize(self, value):
        ...
        if isinstance(value, list):
            return ", ".join(reversed([self._recursive_humanize(sub_value) for sub_value in value]))
        return value.native
...
class ExtendedNetworkAddress(Choice):
    _alternatives = [("e163_4_address", E1634Address), ("psap_address", PresentationAddress, {"implicit": 0})]
...
class PolicyInformation(Sequence):
    _fields = [("policy_identifier", PolicyIdentifier), ("policy_qualifiers", PolicyQualifierInfos, {"optional": True})]
